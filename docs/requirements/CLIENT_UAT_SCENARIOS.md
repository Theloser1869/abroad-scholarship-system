...
**Code-inferred assessment:** All entities real and wired to live endpoints, including the full CommissionTransaction lifecycle (create→confirm-eligibility→calculate→approve→pay). ~~GAP-011 (Partner Program ID prefix bug) still lands on this scenario, untouched by remediation.~~ **GAP-011 RESOLVED 2026-08-27** — Partner Program ID now correctly generates as `PP-CC-NNNNN-NN`; 5 new e2e tests, and separately confirmed live in the browser (created a real Partner Program via the UI for a real French partner, DB showed `PP-FR-00005-01`, test data cleaned up).
**UPDATE (2026-08-25, Client Acceptance Remediation):** GAP-006 partially remediated — `CommissionTransaction.contractId` and `PartnerStudentLink.contractId`/`scholarshipApplicationId` are now real, directly-queryable FKs (e2e-tested, `partners.e2e-spec.ts`, 5 new tests). Which Contract/Scholarship earned a given commission is now a direct join. ~~Visa traceability remains a gap (deliberately deferred, not attempted this phase).~~ **RESOLVED 2026-08-27 (DEC-09) — see below.**
**UPDATE (2026-08-27, DEC-09):** Client confirmed the Visa leg is mandatory for Visa-sourced commissions specifically. `CommissionTransaction.visaId` built, auto-resolved by `resolveSource()` exactly like `contractId`; a Visa-sourced transaction's rule must be FIXED-basis (Visa has no monetary field). All three legs (Contract/Scholarship/Visa) are now directly joinable. 5 new e2e tests, full backend/frontend regression re-run (backend unit 209/209, e2e 556/557 with 1 confirmed unrelated pre-existing flake, frontend 325/325 — see `docs/DECISIONS.md` DEC-16). **This specific leg was also live-browser-verified**, not just e2e-tested: created a real FIXED-basis `CommissionRule` for a real partner through the actual "Tạo quy tắc" UI, created a `CommissionTransaction` with `sourceType=Visa` against a real fixture Visa through the actual "+ Giao dịch" dialog, confirmed the detail page rendered a new "Visa" row with a working link to the real Visa record, ran it through the real "Xác nhận đủ điều kiện" → "Tính toán" buttons and got the correct calculated amount (75,00 USD), and independently confirmed `visaId`/`contractId`/`studentId`/`caseId` in the DB directly. Test data (the rule and the transaction) deleted afterward.
**What this live check did NOT exercise:** it reused an existing Partner/PartnerProgram/PartnerStudentLink fixture rather than creating those from scratch through the UI, and did not walk the transaction through Duyệt (approve)/pay. Those steps, plus the Contract/Scholarship legs and the rest of this scenario's own sequence (Create Partner → add Partner Program → link a Student), remain e2e-tested only, not live-browser-walked.
**Status:** ~~NOT_TESTED (live browser).~~ **PARTIALLY live-browser-verified (2026-08-27) — the Visa-leg traceability piece specifically; see above for exact scope.** e2e-TESTED (API-level): PASS for Contract/Scholarship/Visa traceability. Partner Program ID prefix bug (GAP-011): RESOLVED, live-verified. The full create-Partner→Program→Link→approve→pay sequence for this scenario is still not live-browser-walked end-to-end.
